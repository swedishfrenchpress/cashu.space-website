...
- The hero ASCII field (`src/lib/ascii/`) is full-bleed and morphs on a 76s loop: terrain → vault door → blind-signature round trip → terrain, with a pointer lens; the vault lands by 16s so a first read sees it. Two DESIGN.md §4 rules were amended for it on 2026-08-14 (Fold-Line, Honest-Network) — read those entries before changing the hero's geometry, mask, or scene list. `terrain.ts` and `vault.ts` are pinned by parity fixtures in cashubtc/wallet; retuning their constants is a keep-in-lockstep edit with that repo.
