...
Motion is permitted under one condition: it must depict real protocol structure, or be plain material carrying no assertion at all. The hero's ground is a **wide dot figure** — the `thinking-orbs` `listening` mode, a stack of latitude rings whose radii swell on a slow wave, built at a fixed geometry scalar and remapped into an ellipse roughly twice as wide as it is tall. It is a band across the bottom third of the section, not a ground filling it: the copy sits on open paper and the figure crops past the closing hairline. It takes the rule's *second* branch — it asserts nothing, so it claims nothing. The Bitcoin mark surfaces out of it and recedes again on a 72-second cycle, a constellation of the field's own marks, never a logo animation and never a coin. It does not answer the pointer and has not since the contour lens was removed 2026-08-16. The field is weather, not a control. Never a decorative flow, never a connection the protocol doesn't make, never a hover response. This **replaced the live ASCII field on 2026-08-16, on the user's direction**; see the Honest-Network Rule in §4 for what that cost, and `src/components/hero-orb.tsx` for the figure itself.
...
*Amended 2026-08-16, on the user's direction, with the figure it masks.* The ASCII field is gone and the hero's ground is a single wide dot figure (§1). **The rule's substance is unchanged for the third time — only its mechanism is.** The mask was two gradients intersected; it is one. The elliptical quiet zone was written for an *even* texture: a glyph grid has no density of its own, so the mask had to supply the quiet around the centred copy. The dot figure has density of its own — and, more to the point, it no longer sits behind the copy at all — so a centre-dim would have quieted the figure's most open region and sat squarely on the stage where the ₿ resolves. It is retired, and `mask-composite` with it. The vertical fade does the whole job: transparent to 9%, 0.10 at 46%, 0.42 at 68%, full by 86%. The 9% flat run is still load-bearing twice over and `MASK_DEAD_TOP` in `hero-orb.tsx` still has to track it.

**The matched pair held and moved together, which is the point of the pairing.** The figure stages centred, its ellipse at 95% of the hero height with a vertical semi-axis of 30%, so its top arc lands around 65% and its lower arc crops past the closing hairline — the vault's crest composition, carried onto the thing that replaced it. "Title at the top, horizon at the bottom" is now literal rather than mask-enforced. The ₿ stages lower still, in the band between the *measured* bottom of the copy block and the fold, and stands down entirely when that band is too short to hold it: the same legibility stand-down the round trip took below 768px, for the same reason.
...
2. **The figure and the ₿ move off-axis.** The mask is a plain vertical fade now (see the Fold-Line Rule), which is alignment-agnostic — but the *figure* is centred on the page axis because the copy is, and the ₿ stages on that axis too. Against left-aligned copy both have to move off-axis with it, or the figure sits behind the column instead of framing it.
...
*Recorded waiver, same date; settled 2026-08-16.* The `$ ¥ €` glyphs on high contours and `₿` at the peaks (`terrain.ts`) depicted no protocol structure; the 2026-07-26 critique flagged it, and it was retained as the site's own voice with the ₿ density deliberately increased. The fiat glyphs went with the field they lived in and are deferred at most — and a note for whoever revives them: **the shipped GT-Standard trial faces carry none of `$ ¥ € ₿`** (271 codepoints, cmap-verified), so any of them has to be drawn, not typed. The ₿ survived on the same waiver, synthesised from the font's own `B` plus two rules: it is the mark of the money the protocol is denominated in, which is a fact about Cashu rather than a claim about its mechanism.
...
- ~~**The hero is untouched.** Its field is the site's own drawing and the round trip in it is real protocol structure. Nothing here licenses swapping that for a library.~~
  ...
  **Retracted 2026-08-16, later the same day, on the user's direction — objections raised and overruled.** The hero's ASCII field was deleted and its ground is a `thinking-orbs` figure too (§1; `hero-orb.tsx`; `src/lib/ascii/` and `ascii-field.tsx` are gone). What it cost is worth stating plainly, because the clause above existed to prevent exactly this: **the site lost the one piece of motion on the page that *was* real protocol structure** — the blind-signature round trip, which this rule's own 2026-08-14 amendment called "the scene that earns the motion". Nothing on the homepage depicts the mechanism now. The hero's figure clears the rule on the *other* branch, the one the four plates stand on: an abstract figure that asserts no structure is material, not a claim. It is admitted as material, and material is all it is.
  ...
  - **The ₿ in it is a mark, not a mechanism and not a coin.** It surfaces out of the field's own marks and recedes on a 72-second cycle. It never spins, never scales, never catches a light, and is never a disc. PRODUCT.md's first anti-reference is "animated 3D coins"; those four prohibitions are the boundary that keeps this on the right side of it.
