...
2. **The field's quiet zone moves off-axis.** It is a centre-anchored ellipse now (see the Fold-Line Rule) because centred copy sits on the page axis; against left-aligned copy it would quiet the wrong ground — and the scene stage at bottom-centre moves with it, or the figures collide with the copy column.
...
- **One ground, two artifacts.** Both views sit on the same ASCII sheet: `--fig-ground` (Chalk in light, Band in dark) under a frozen sample of the homepage contour field. The field attenuates radially toward the centre — the ground yields under the artifact — and each panel/view uses a nearby fixed terrain time. Switching Figure/Code interpolates between those times over 560ms: contours migrate cell by cell, so glyphs promote, demote, and disappear into the new sample rather than crossfading as a bitmap. The renderer returns to idle at the endpoint; interrupted toggles continue from the current interpolated time. Reduced-motion changes the sample immediately. All depth in the panel comes from value steps, keylines, glyph density, and masks — never blur, translucency, or shadow; the one glass surface remains the navbar. Neither ground is the Twilight Stack: no bloom-to-black, no grain. Twilight stays footer-reserved.
- **The figure is a published plate, not a file.** Card fill on the ASCII sheet; a hairline keyline whose sides, lit top edge, and seated bottom edge come from `--fig-edge`/`--fig-lit`/`--fig-shade` — borders, so the No-Shadow Rule holds. **The keyline set never takes `--hair`.** It stays one tone off the sheet so the plate reads as a distinct object over the glyph field. Then an RFC-style caption *below* the plate — a thesis sentence in GT-Standard body-small. The caption is the site's own voice describing the figure; the plate interior speaks machine notation — Mono field names at micro tracking, Pixel amounts at `1.5rem` as the plate's one scale jump. Plates size to their content; a plate never pads itself tall.
...
- **The toggle** is the feature-nav register one size down: uppercase GT 500 at `0.75rem`, `letter-spacing: 0.06em`, active segment inverts to Ink/On-Ink. It carries its own Card ground and a `--fig-edge` rim so it stays distinct from the ASCII field. Flat and square like the rest of the system.
