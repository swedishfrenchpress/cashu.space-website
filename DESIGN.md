...
**The Focus-Pull Rule — RETIRED 2026-08-21, on the user's direction.** *(Blur returned the same day at 4px as one component of the stock entrance — see the Stagger Rule below. What stays retired is blur as a bespoke gesture on display type, and blur as depth.)* *Added 2026-08-19 over a recorded objection; narrowed 2026-08-20 when the hero withdrew ("unelegant and sloppy"); deleted 2026-08-21 when the last two consumers — the section groups and the `/wallets` route header — took a travelling edge instead and `.reveal--focus` was left with zero (that edge, and the line rise that replaced it, were both gone within hours — see the Stagger Rule). The whole system went with it: `--reveal-blur`, `--reveal-blur-section`, `--reveal-focus-span`, `--reveal-focus-span-section`, both `reveal-arrival-*-focus` keyframes, and the `focus` prop on `Reveal`. See the Stagger Rule below for what stands there now.*

*The narrowing had been running for two days and this is the end of that line rather than a reversal of it. The recorded objection — that blur is the one gesture in the system which does not rhyme with the others, and belongs by family to the glass the site spent a day removing — was marked right on 2026-08-20 and then applied only to the hero. Applying it to the rest was the outstanding half. The rule's own text below flagged the last piece as an open question in as many words: "the open question there is whether that header should keep a focus pull at all, not what number it should run at." It should not.*

*Verified after removal, production build at 1440×900: **0 elements on either route hold a `filter: blur()` or a `backdrop-filter` at any point in the page's life**, and the only `filter` values left anywhere are the pre-existing, deliberate `grayscale(1) contrast(1.05)` on press photography and `brightness(0)` on the flattened publication marks. `will-change` held by settled elements went to 0. Motion's own text-animation guidance independently reaches the same conclusion on cost — "blur is a classic trap", because small text-sized layers, blurred, overlap and cost far more GPU surface than blurring one root layer, and every entrance element on the site was paying that.*

**The rule as it stood, kept for the record.** **Type may arrive fractionally out of focus and resolve. Nothing may rest out of focus.** `filter: blur()` is admissible only as a property of an entrance — it starts with the settle, it clears before the settle finishes, and it is gone, not zeroed, when the element lands. Everywhere else on the site blur remains forbidden. *(That last sentence is now the whole rule, without the carve-out.)*
...
**The Stagger Rule.** *Added 2026-08-21, on the user's direction, replacing the Line-Rise Rule and every bespoke entrance before it.* **The site has one entrance and it is a plain one:** `opacity 0 → 1`, `y 40 → 0`, `blur(4px) → none`, on a `stiffness: 120, damping: 20` spring, children staggered 0.1s apart. It is Motion's `hero-stagger` example, adapted — the animation only, none of its chrome.

***Six entrances were built for the hero headline. Five were rejected, and the sixth is the one that stopped trying.*** This table is the most useful thing in §4, because it is the argument:

| Treatment | Verdict | Date |
| --- | --- | --- |
| `hero-cipher.tsx` — decrypt out of hex | "corny and too on the nose" | 2026-08-19 |
| `.reveal--focus` — blur in and resolve | "unelegant and sloppy" | 2026-08-20 |
| `.hero-glyph` v1 — hard `clip-path` per glyph | "stiff and mechanic and generic and robotic" | 2026-08-20 |
| `.hero-glyph` v2 + `headline-set` — gradient sweep | "corny" | 2026-08-21 |
| `.line-rise` — rise through a static window | "too corny" | 2026-08-21 |
| **`Stagger`** — opacity, y, 4px blur, spring | **shipped** | 2026-08-21 |

The five rejected share no mechanism — a substitution, a filter, a mask, a wipe, a translation. What they share is that **each was invented for this site**, and the headline visibly performed its own arrival. The sixth is a stock entrance used on thousands of pages, and that is the point: it does not draw attention to itself, because there is nothing about it to notice. **Do not replace it with a seventh bespoke treatment.**

**Blur is back, and the retirement above is therefore partly reversed.** The Focus-Pull Rule's entry still stands as history and its reasoning still holds for *depth* — no standing translucent surface, no glass — but "there is no blur anywhere on cashu.space" was true for about an hour and is not true now. What is different is the magnitude and the argument: the focus pull ran **10–18px** on display type as a bespoke gesture and read as sloppy at 9rem; this runs **4px flat**, as one component of a stock entrance, and at 144px it is a softness rather than a smear. The mechanical term survives unchanged and is the one thing that must not be relaxed: **it ends on `filter: none`, never on `blur(0px)`.**

**That term costs one line and it was measured, not assumed.** Motion can only tween to `blur(0px)` — `none` is not a filter it can interpolate to — so a settled item would keep a compositor layer and grayscale antialiasing on its type for the life of the page. The item clears the inline filter itself on completion, and the clear must be **deferred by one turn**: Motion writes the animation's final value *after* `onAnimationComplete` fires, so clearing synchronously is silently overwritten. Both states were measured — synchronous left 28 elements holding `blur(0px)`; deferred leaves 0, on all three routes, after scrolling every section. `y` needs no equivalent: Motion already returns the literal string `"none"` when every transform value is at its default.

**The hidden opacity is 0.02, not 0**, which is the one other change from the example. Chrome will not take an element as an LCP candidate if its only main-thread paint is fully transparent, and a composited opacity animation never repaints on the main thread. Measured on this site: from 0 the arrival cost LCP 2244ms; from 0.02 it was 776ms. The two are visually identical.

**`y` is per surface, and the example makes it a prop for the same reason.** 40px under 9rem display type is right; 40px on a thirteen-row wallet directory is a lurch, so those take 24px and the masthead takes 0 — a bar that slides down from above the viewport edge is a cookie banner.

**Three costs, recorded rather than hidden.** *(1)* The entrance is now a **runtime**, not a stylesheet, so it is hydration-gated where the old `.reveal--arrival` was not. *(2)* Motion serialises the `initial` variant into the **server-rendered `style` attribute**, so a visitor with scripting off receives markup that is already hidden and has nothing to animate it back. No CSS selector can undo an inline style, so the rescue is a `<noscript>` block in `layout.tsx` — one of exactly two uses of `!important` in this codebase, deliberately, because it is the only thing that can win there. *(3)* The 1.5s failsafe no longer *retreats* (dropping `html.js` cannot reach an inline style) but **repairs — and, corrected 2026-08-26, repairs only on evidence of absence.** As shipped on 2026-08-21 it cleared inline opacity/transform/filter off every still-hidden item unconditionally, and that was a bug on every normal load: Motion animates from its own recorded values, never from the DOM, so the first `whileInView` frame wrote the hidden state *back* onto content the reader was already looking at — appear, vanish, reappear, deterministically, for every section reached after 1.5s. The repair is now gated on `data-stagger-live`, a marker `stagger.tsx` sets from a **delivered animation frame** (hydration alone is not evidence — the backgrounded-tab case is hydration completing while rAF stays paused, and rAF never fires hidden, so a hidden tab *defers* the check to `visibilitychange` plus the same 1.5s grace instead of repairing into the dark). And the instrument is no longer a style wipe but the `<noscript>` rule injected as a live sheet — the second `!important` — so every failure mode, late hydration after a repair included, degrades to "content visible, entrance suppressed", never to a flash. Verified on the production build at 1440×900 over per-frame opacity traces: normal load, below-fold items still hold `opacity: 0.02` at t=2.3s and no item ever drops below 0.9 after revealing (1106 frames, homepage; 785, `/wallets`); with all `_next/static/chunks/*` blocked, every item is readable at t=2.3s via the injected sheet; a load in a hidden tab sets no marker, fires no repair, and plays clean entrances after foregrounding.

**The Curtain Rule.** *Added 2026-08-21, on the user's direction.* **A route change is a wipe, not a cut.** Motion+'s `useCurtains` covers the viewport with a `clipWipe` in Ink, runs the navigation while it is hidden, and lifts once React has committed *and painted*. Cover is `--dur-base` (220ms), reveal is `--dur-nav-wipe` (460ms) — existing tokens, and the asymmetry is the Sheet Rule's reasoning at viewport scale: the cover answers a click already made, the reveal is presenting a page and has nothing to race. Direction is left to right with `directionMode: "normal"`, so cover and reveal are **one edge crossing the viewport twice in the same direction** rather than a door opening onto the same room.

**It is not a new gesture.** `clipWipe` is, in Motion's words, "a wipe whose edge is a clip on a static panel, so the overlay itself never moves" — the identical mechanism to the masthead plate, the hero hairline and the `?` sheet, at the largest scale the site has. `.motion-curtain` is `--ink`: flat, opaque, square. A curtain that let the outgoing page show through would be the glass this site removed.

**Three terms.** **Same-document anchors are not navigations** — the test is the pathname, not the href, so `/#why-cashu` wipes from `/wallets` and merely scrolls from the homepage. **Modified clicks are left to the browser**, since cmd/shift/middle-click open somewhere the reader is not. And **a hidden document gets a plain navigation**: Chrome freezes `document.timeline` and rAF in a backgrounded tab, so the cover's `finished` promise never settles — measured, the animation sat at `currentTime: 0` in `playState: "running"` while 818ms of wall clock passed and the route never changed. *(That guard covers a click dispatched while hidden. A reader who clicks on a visible tab and then switches away gets a stall that resumes on return, which is how every CSS animation on the site already behaves and is deliberately left alone — forcing it to completion would mean returning to a page that changed under them.)*

**The Uncovered-Footer Rule.** *Added 2026-08-21, on the user's direction, correcting the footer reveal shipped the day before.* **The Twilight Stack is uncovered, not faded.** The plate is full-value Ink, sharp and full-bleed at every scroll position; the page's opaque cover slides off it, and the only scrubbed property is a **24px lift on the footer's contents** that lands when 60% of the plate is uncovered.

*What it corrects, because all three faults were structural rather than tuning.* The first version drove `opacity 0→1`, `scale 0.94→1` and `blur 6px→0`, and its own commit recorded the verdict: "reads as frosted glass on a floating card". Measured live at 1665×1779 it was worse than that. `useScroll` with `["end end", "end start"]` maps progress onto one viewport of scroll after the cover's bottom edge reaches the viewport bottom — but the scroll *remaining* then is exactly the plate's height, so the maximum reachable progress equalled the value the fade was calibrated to. **The plate was mid-scrub for its entire visible life and reached its designed state at exactly one scroll position**: 116px from the bottom it still held opacity 0.847 and a 0.92px blur. On top of that, fading Ink over Paper passes through grey, and any scale below 1 on a full-bleed plate exposes the page ground down both edges — ~50px per side early in the scrub, which is the definition of the floating card a full-bleed surface must not be. Verified after: **0.0px of Paper exposed, opacity 1, filter none at every position**, and the lift ends on `transform: none` natively — Motion returns the literal string `"none"` when every transform value is at its default, so the epsilon the old `filter` needed has no analogue.

*24px is `travel.enter`, not `travel.section`'s 48.* 48 is sized to move a whole surface; here the surface is fixed and only what is printed on it lifts, and 48px of type sliding inside a stationary Ink band reads as two planes — the floating card this exists to delete. 24 also sits one step above `--reveal-rise-section`'s 20px: the footer is heavier than a section, not three times heavier.

**The Thrown-Rail Rule.** *Added 2026-08-21, on the user's direction.* **The one component the reader manipulates directly responds like an object.** The press rail can be grabbed and thrown: snap is suspended for the length of the drag, release projects the coast (`--dur-base` of the release velocity), rounds it to the nearest snap point, and springs there on the theme's `ui` transition — so restoring snap is a no-op rather than a correction anyone sees.

**It is additive, which is the whole design.** The track stays a real scroll container: touch, trackpad, keyboard and the pager all keep working and none of them go through this code. Motion's `drag` was the obvious reach and is **deliberately not used** — it wants a transform-driven rail, and converting to one would trade native touch and keyboard scrolling for a mouse affordance. The handler attaches only on `(pointer: fine)`, because coarse pointers already have platform momentum that is better than anything here. **A drag swallows the click it ends on** (every card is a link covering most of the track, so a throw finishing over a card would otherwise open a news site) with a 4px threshold that keeps an ordinary click working. This is also the first real consumer of the Motion UI theme provider, which until now supplied a context nothing read.

**The Adaptive-Caret Rule.** *Added 2026-08-21, on the user's direction, after Motion's `cursor` example.* **The browser cursor is replaced by the site's own, and its only idea is that it measures what it is over.** Motion+'s `Cursor` in replace mode (`site-cursor.tsx`), pinned to the pointer with no spring, in three states: a **12px square** at rest, a **28px square** over a link or button, and — the example's whole subject — a **4px caret the height of the type** over anything selectable, so it stands 129px tall on the hero headline and 16px in the footer disclaimer. On a site whose register is a published specification, a pointer that reports the measure of the text under it is the one custom cursor that is about the document rather than about itself.

**One value, every ground, and no table to maintain.** The cursor is `#ffffff` under `mix-blend-mode: difference`, so it renders `|ground − 255|` at every pixel it covers: pure `#000000` on Paper — which is `--edge`, the token the site already reserves for pure-black accents — and near-white on the masthead, the footer plate, the protocol-parts column, the Spec pane and the greyscale press photography. The alternatives were a `data-cursor-zone` value per dark surface (five entries today, and a wrong one the day a sixth is added) and a cursor with a keyline, which is closed off because `Cursor` morphs by **scaling** and a scaled border visibly thickens and thins through the transition. **This does not widen the No-Colour Rule:** on a monochrome ground the difference of two greys is a grey. It is not glass either — nothing here is translucent, blurred or soft. It is an inversion, which is a printer's mark, not a material.

**Three guards, all measured, none of them a preference.** `(pointer: fine)` — the component is not mounted at all on a touch device, the same argument that scopes the rail's `grab`; verified under device emulation: no element, `cursor: auto`. `prefers-reduced-motion` — return null and let the platform cursor stand, read through a **live-tracked** `matchMedia` rather than Motion's `useReducedMotion`, which is `useState(initial)` and reads the query exactly once (the correction `button-cipher.tsx` took on 2026-08-20); verified in both directions, including toggled mid-session. And nothing renders until the pointer first moves, which is Motion's own `useHasPointerMoved`: a keyboard visitor never sees this component, and there is no square parked at (0, 0) on load. The morph runs on the theme's **`snap`** token — its second consumer after the rail — so the cursor changes shape with the same physics as everything else that answers a pointer, rather than on Motion's own one-off 150ms curve, which would have been the site's third easing.

**What it costs, recorded rather than hidden.** Replacing the cursor means an injected `* { cursor: none !important }`, so **every `cursor` declaration on the site stops being visible.** Only one carried information — `.press-track`'s `grab` — and it is repaired in the cursor's own material: `data-cursor-zone="rail"` sizes it to a **28×8 bar on the rail's own axis**, scoped to the `default` type because that is exactly the geometry `grab` had (the cards are `<a>` elements and the UA's `pointer` always beat the track's `grab` over them). The second cost is the `?` sheet: **a modal `<dialog>` paints in the top layer and this element does not**, so the sheet would have had no cursor at all. Two rules in `globals.css` fix it with no coupling — the platform cursor comes back document-wide while `.keymap-help[open]` matches, and the custom one is hidden underneath (`visibility`, not `opacity`: Motion drives opacity from an inline style). **`cursor: revert` is the wrong instrument there and looks like the right one** — `cursor` is inherited, `revert` rolls an author declaration back to a UA origin that states nothing for these elements, and the property then falls through to the inherited `none` it was meant to undo. Measured: `revert` left the sheet computing `cursor: none`; `auto` is what works.

**The blend mode is free and the cursor nearly is.** Measured over 120 pointer moves across the hero at 1440×900, main-thread script and style recalc: `difference` 49.1 / 40.3ms script and 18.2 / 17.3ms style, `normal` 45.7ms and 17.0ms, cursor removed 32.7ms and 9.0ms. The blend costs nothing over no blend; the component's own position and layout work is about 10ms of script and 8ms of style per 1.7 seconds of continuous movement.
...
#### The Set-Once Rule, fifth amendment: the ground arrives, from the bottom left

*Amended 2026-08-21, on the user's direction, after [motion.dev/examples/react-staggered-grid](https://motion.dev/examples/react-staggered-grid).*

**The cipher field arrives on a wavefront that crosses it from the bottom left, and then it is the same still field it has always been.** Until now it was the one element in the hero with no entrance at all: the hairline drew, the three stagger items rose, and the field simply existed from whichever frame `document.fonts.ready` happened to resolve on. That is not stillness, it is a pop.

**The mechanism is Motion's, and that is the point.** The example gives each cell in a grid a delay equal to its physical distance from an origin cell, plus a jitter. Same idea here, with one origin — the canvas's bottom-left corner, so the field grows up and to the right, out from behind the type and off the right edge, which is the direction every other wipe on this site runs. What could **not** be ported is the per-cell spring: Motion's cells fade and scale, and a cell here is one bit. So the wavefront multiplies **occupancy, not alpha** — a cell is absent, then it is fully set, and the softness lives in the order the cells cross over rather than in any of them being half-drawn. That is the field's founding lesson (fading type with alpha greys the letterforms into a smudge) applied to its entrance, and it is why the ramp has to be wide: it is the only thing making this a front rather than a travelling edge, and the site already has three of those.

**It is not a second entrance.** The progress runs on the hero's own spring — the `stiffness: 120, damping: 20` exported from `stagger.tsx`, the same physics the headline, deck and CTAs settle on. The field is not a Motion element and cannot be a `StaggerItem`, but it can share the spring and the moment, and the distance term in the shader supplies the stagger a `StaggerItem` would have got from its parent. That is what keeps this inside the Stagger Rule's *one* entrance instead of being a seventh treatment invented for the hero.

**Four terms readmitted a ground at all (the first amendment), and all four still hold.**

1. **`uReveal` is not the banned time uniform.** That term is about a *clock* — a value that keeps moving after the reader has stopped. `uReveal` is a one-shot progress: 0 to 1 at arrival, then 1 forever. The arithmetic is arranged so the maximum delay plus the ramp is exactly 1.0, so at rest it resolves to a multiply by one. **Verified: the settled field is byte-for-byte identical to the pre-change build** (same 440×830 clip of the field column, `cmp` clean). If anyone ever wants to feed this a modulo, a sine or a delta, that is the banned uniform and the answer is no.
2. **There is still no rAF at rest.** Motion owns the entrance's frames and stops itself; `window.__heroFieldFrames` measured 42 at 1440×900, then **42 → 42 across a quiet 1.5s**. The counter's contract is unchanged — it proves there is no loop *at rest*, and an arrival is not rest.
3. **It stops on the resting frame**, explicitly: `onComplete` sets the progress to 1 and renders once more, because the spring's last sample is 1 to within a rounding error and "within a rounding error" is the difference between the settled field and a settled field missing the far corner's last ring of cells forever.
4. **Gating is per tier, as before, but the entrance's tier is not the plume's.** The plume needs a fine pointer because it needs something to *cause* it; an arrival is caused by the page loading, which every device does. So the wavefront is gated on **reduced motion alone** and a phone gets it. Verified under `prefers-reduced-motion: reduce`: canvas present, **0 frames** — the field is simply there.

**The wave is normalised over the field, not over the canvas, and that is the difference between a wave and a flash.** The first build divided by the raw canvas diagonal. In column mode everything left of the type's right edge is cleared, so at 1440×900 every visible cell sat between 1088 and 1698 device pixels from the origin — the last fifth of the sweep. Measured: the whole right-hand column crossed the threshold inside **13% of the progress** and read as the field simply appearing, which is what it did before any of this existed. The CPU now subtracts the dead run (`revealNear`) and the shader spends the whole 0-to-1 on cells that are actually drawn. It is computed in `measureSafeBox`, beside the branch that picks the mask geometry, because the two modes give it different answers — column mode's cleared zone runs the full height so the first drawn cell is at the boundary, box mode's floats clear of the corners so there is no dead run at all.

**One honest limit.** The entrance fires when the font resolves, which is the busiest moment on the main thread; on a software rasteriser the spring's first two samples measured 0.017 and 0.537, i.e. it skipped half the sweep in one frame. That is a frame-budget artifact, not a timing choice, and it is the failure mode to watch on a slow device — the wave degrades toward the pop it replaced rather than toward anything worse. **`document.hidden` is checked at the moment the entrance would start, not at mount**, because a background tab freezes `document.timeline` (measured on the route curtain: `currentTime: 0` while 818ms passed). A frozen spring would hold `uReveal` at 0, and `uReveal` at 0 is a blank hero. Not visible, not animated, just arrived.
