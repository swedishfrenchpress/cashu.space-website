...
So the composition inverts: **the title block centres in the section, and the closing hairline *is* the horizon.** `.hero-spec__inner` takes `margin-block: auto`, which in the section's flex column splits the free space above and below it; the section's padding is asymmetric (`clamp(72px, 9vw, 144px)` top, `clamp(104px, 12vw, 200px)` bottom) so the block seats a few percent **above** true centre, because a block centred mathematically reads low. Measured at 1440×900: 137px above, 180px below, the block 5.1% above centre. The lift is done with padding and not a transform — a transform on the hero would establish a containing block and can land display type on fractional pixels.
...
3. **The headline's width escape is now redundant and can be retired.** It sizes off the viewport rather than the page shell (`min(calc(100vw - 2 * var(--page-x)), 1440px)`) because the shell capped it at 1088px. The "~1283px to break in two" figure this item used to cite was **wrong** — the real requirement is 1513px for `OPEN SOURCE ECASH`, which no cap reachable here supplies, and §3 now records why two lines are impossible at the Display scale at all.

   Once the rag is authored rather than balanced (§3), the escape stops buying anything: the widest line is `OPEN SOURCE` at 1003px, and the shell's 1088px already holds it. Measured at 768/1024/1280/1440/1600/1920/2560 with the width forced back to the shell cap: **three lines at every one of them, identical to the escape.** Because the lines are centred and none of them fills either box, retiring it is a visual no-op.

   It is left in place rather than removed, deliberately: `.hero-spec__content` is `align-items: center`, so the h1 needs *some* explicit width or it shrinks to max-content and `.hero-spec`'s `overflow: hidden` would clip it on narrow viewports. Retiring the escape means replacing it with the shell expression, not deleting the property — a small change with a real regression surface and no visible payoff, so it wants its own pass rather than riding along with the rag.
