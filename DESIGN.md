...
**The timing is arithmetic, not taste.** Start 120ms — what the headline's reveal already carried. Step 30ms, about two frames at 60Hz; under roughly 25ms adjacent glyphs start on the same frame and the cascade collapses back into a single sweep, which is the individuality the whole change is for. Duration 110ms per glyph, deliberately close to the step: a duration much larger than the step puts a dozen glyphs in flight at once and the eye reads one soft band travelling rather than characters arriving. The last index is 29 — 26 glyphs and four spaces, two of them line ends, each taking a beat so the pass rests where the eye has to travel — so the headline finishes at `120 + 29×30 + 110 = 1100ms`, **exactly when the closing hairline finishes its draw.** The rule's "by roughly 1.1 seconds nothing is moving" is unchanged; the hero's two remaining motions now land together instead of one trailing the other.

**`backwards`, not `both`, on thirty spans rather than one** — the same term the third amendment states, and it matters more at this count: `both` would leave thirty permanent stacking contexts and compositing hints under the largest type on the site. Verified after settling: 0 of 26 glyphs hold a `clip-path`, a `filter` or a `will-change`.

**Measured** (Slow 4G, 4× CPU, 412×915 at DPR 2, three runs each). LCP 644 / 648 / 656ms after, against 648 / 656 / 664ms before; CLS 0.00 throughout. **The LCP element changed from `H1` to `P` and that is the finding worth recording**: a fully clipped headline is not an LCP candidate, so the deck takes over — at the same millisecond, because both are painting off the same stylesheet at parse time and the deck still carries the 0.02 opacity floor. The metric is unchanged. What genuinely costs something is legibility of the headline itself, which completes at parse + 1100ms instead of parse + 740ms; that is what a typewriter is, and it was asked for.
