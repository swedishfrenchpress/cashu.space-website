...
4. **Editorial pacing.** Massive display type at the hero, generous whitespace between sections, long single-column reads. The hero earns its size; nothing competes with it. Sections breathe.
   ...
   *Scoped 2026-08-14, on the user's direction:* the homepage hero is **centred**; everything else on the site stays left-aligned editorial. Centring the hero sits closer to the marketing register than the rest of the page, which is a real tension with "editorial-minimal, confident through silence" above. It was weighed and chosen. Treat it as bounded to that one surface — see the Centred-Hero Exception in DESIGN.md §4 for what has to move if it is ever reverted.
