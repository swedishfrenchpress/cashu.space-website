...
// Vertex shader source code - This controls how vertices (points) are positioned
...
void main() {
  ...
  // When mouse is active, create displacement effect
...
// Fragment shader source code - This controls the color of each pixel
...
// Random function for dot placement
...
void main() {
  ...
  float lightAdjustedDotSize = dotSizePixels * smoothstep(0.2, 0.8, adjustedBrightness);
  float darkAdjustedDotSize = dotSizePixels * smoothstep(0.15, 1.0, adjustedBrightness);
...
// Helper function to load and bind an image texture
...
export default function ShaderEffect({
  ...
  useEffect(() => {
    ...
    // Create mesh grid (50x50 grid for smooth deformation)
    ...
    // Create buffers for position, texture coordinates, and indices
    ...
    function render(time: number) {
      ...
      // Update lagged mouse position for smooth following
      laggedMouseRef.current = {
        x: laggedMouseRef.current.x + (mouseRef.current.x - laggedMouseRef.current.x) * 0.15,
        y: laggedMouseRef.current.y + (mouseRef.current.y - laggedMouseRef.current.y) * 0.15,
      }

      // Update mouse activation smoothly
      const targetActivation = mouseActiveRef.current
      mouseActivationRef.current += (targetActivation - mouseActivationRef.current) * 0.06
      ...
      gl.uniform1f(darkModeLocation, darkMode ? 1 : 0)
      ...
      gl.uniform2f(laggedMouseLocation, laggedMouseRef.current.x, laggedMouseRef.current.y)
  ...
  }, [imageSrc, darkMode, width, height])
